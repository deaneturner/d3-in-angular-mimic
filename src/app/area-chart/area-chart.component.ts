...
export class AreaChartComponent implements OnInit, OnChanges {
  ...
  bins!: d3.Bin<number, number>[][]; // Array of frequency distributions - one for each area chart
  paths!: any[];
  // @ts-ignore
  area!; // For D3 area function
  // For D3 area function
  // For D3 area function
  ...
  private createChart(data: number[]) {
    ...
    // d3 area and histogram functions have to be declared after x and y functions are defined
    this.area = d3.area()
  ...
  private processData(data: number[]) {
    this.bins = [];
    data.forEach((row) => {
      this.bins.push(this.histogram(row as any))
    });
  }
